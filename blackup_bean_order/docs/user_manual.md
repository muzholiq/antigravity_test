...
Antigravity에서 이렇게 말하세요:

```
"서면본점 Sheets URL 등록해줘"
```

브라우저에서 해당 시트를 열어두면, 에이전트가 URL을 자동으로 읽어옵니다.
...
"에티오피아 예가체프 전체 15kg" — 로스팅 계획의 핵심 데이터.

### 섹션 3: 배송 스케줄

배송희망일 기준으로 정렬. 긴급 표시 포함.
...
| "YAML 오류" | settings 파일 손상 | "settings 확인해줘"라고 에이전트에게 요청 |
