...
> **Version 1.2** | 2026-02-16 (설치 가이드 및 v1.5 프리뷰 포함)
>
> 이 문서는 **로스터(관리자)**와 **매장 바리스타** 두 사용자 그룹을 위한 실무 가이드입니다.
> 설치 및 초기 설정은 별도의 [설치 가이드](setup_guide.md)를 참고하세요.
...
| 2 | **구글 서비스 계정 키** | `service_account.json` 파일 (설치 가이드 참고) |
| 3 | **이 프로젝트 폴더** | `blackup_bean_order` 폴더 통째로 준비 |
...
또한, **서비스 계정 이메일**을 **편집자(Editor)**로 초대해야 합니다.
...
|---|---------|------|
| 1 | 바리스타에게 **편집자** 권한 부여 | ⬜ |
| 2 | **서비스 계정 이메일**을 **편집자**로 초대 (`settings.yaml` 참고) | ⬜ |
| 3 | 바리스타가 실제로 데이터 입력 가능한지 확인 | ⬜ |
| 4 | 에이전트가 시트 데이터 읽기/쓰기 가능한지 확인 | ⬜ |

> **서비스 계정 이메일 확인법**: `config/service_account.json` 파일을 열어 `client_email` 항목을 확인하세요. (예: `antigravity-agent@project-id.iam.gserviceaccount.com`)
...
| 증상 | 원인 | 해결 |
|------|------|------|
| "시트 접근 불가" | 권한 설정 문제 | 서비스 계정 이메일을 `Editor`로 초대했는지 확인 |
...
| 원두명 | 정확한 원두명 |
| 수량(kg) | 양의 숫자 (소수점 1자리 가능, 예: 0.5) |
| 배송희망일 | 주문일 + 최소 2일 이상 (`일요일` 제외) |
...
| 분쇄여부 | 홀빈 / 분쇄 |
| 긴급여부 | Y (긴급) / N (일반) ※긴급은 최우선 노력일 뿐, 확정 보장이 아님 |
| 비고 | 특별 요청 사항 |
| 주문자 | 담당자 이름 |
| 연락처 | 담당자 연락처 |
...
- **M열 (수령확인)**: 물건을 받으면 박스를 체크(☑️)해주세요.
  - 주문 접수 후 **2일** 내 미수령 시 로스터에게 경고 알림이 갑니다.
...
| 배송희망일: 일요일 | 배송희망일: 평일/토요일 | **일요일 배송 불가** |
| 수량: `다섯` | 수량: `5` | 숫자만 입력 |
...
> **🚨 절대 금지**: 시트 구조를 임의로 변경하지 마세요!
> - 컬럼 순서 변경 (X)
> - 컬럼 삭제/추가 (X)
> - 1행(헤더) 수정 (X)
> - 필터/정렬 후 원복 안 함 (X)
>
> 위 작업은 에이전트가 데이터를 읽지 못하게 만들어 **주문 누락**의 원인이 됩니다.
