...
> **보안 권고**: '링크가 있는 모든 사용자에게 편집 권한' 옵션은 편리하지만 보안에 취약합니다.
> **실운영 시 권장사항**:
> 1. 에이전트 전용 Google 계정 생성 (예: `blackup.agent@gmail.com`)
> 2. 해당 계정만 `Editor`로 초대
> 3. `settings.yaml`의 `read_only: true` 옵션 확인
