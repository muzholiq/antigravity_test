...
**처리 중 진행률을 사용자에게 실시간으로 표시**한다 (에이전트 발화 또는 상태 메시지 활용):
```
"총 10개 매장 중 1번째: 로스터리(RST) 처리 중..."
"✅ 로스터리 완료. (1/10)"
"총 10개 매장 중 2번째: 서면본점(SM) 처리 중..."
...
```

각 매장별 처리:
1. `sheet_url`을 **브라우저 도구**로 열기 (접근 실패 시 **1~2초 대기 후 최대 2회 재시도**)
2. 시트의 **첫 번째 탭**만 읽는다 (복수 탭이 있더라도 첫 탭만 처리)
3. 시트 데이터를 **테이블 형태**로 파싱 (13개 컬럼 구조)
4. **`주문일` 컬럼으로 해당 날짜 데이터만 필터링**
5. 중간 집계에 추가
...
for o in orders:
    ...
    bean_totals[key]["store_details"][o["store_code"]] += o["quantity_kg"]
# 출력 예: ETH-YRG 미디엄 35kg (SM 15, HD 10, YS 10)
...
**리포트 최상단에 처리 상태 배지**를 반드시 포함한다:
```markdown
# ☕️ 일일 원두 주문 리포트 (2026-02-15)

> **상태**: ✅ 10/10 매장 성공
> — 또는 —
> **상태**: ⚠️ 8/10 매장 성공 | ❌ 2매장 실패 (을숙도, 울산옥동)
```

**동일 날짜 리포트 존재 여부 확인**:
1. 리포트 생성 전, `run_command`로 파일 존재 여부를 확인한다:
   `ls blackup_bean_order/reports/YYYY-MM/daily_report_YYYY-MM-DD*.md`
2. 파일이 존재하면 사용자에게 확인:
   ```
   "오늘 17:30에 생성된 리포트가 있습니다. 덮어쓸까요, 아니면 (2)로 새로 만들까요?"
   ```
...
- **원두+로스팅 조합별 합산 필수**: 원두 종류와 로스팅 단계별로 그룹화하여 합계 산출
- **매장별 소계 필수**: 각 조합별 합계 옆에 어떤 매장이 얼마만큼 주문했는지 괄호 안에 표기 (수량 내림차순 정렬)
  - 형식: `원두명 로스팅 합계kg (매장1 Nkg, 매장2 Nkg)`
  - 예시: `ETH-YRG 미디엄 35kg (SM 15, HD 10, YS 10)`
- 배송 일정: 배송희망일 기준 정렬 + **매장 주소 포함**

### 섹션 4: 📦 장기 미수령/배송지연 목록 (있을 경우만)
   - **범위**: `A2:M` (헤더 제외, M열 수령확인까지 포함) 또는 데이터가 있는 전체 범위.
   - **데이터 필터링 (두 가지 그룹으로 분리)**:
     1. **금일 주문 (Today's Orders)**: `주문일` == `오늘 날짜`
        - 리포트의 메인 데이터로 사용.
     2. **미수령 주문 (Unconfirmed Delivery)**:
        - `주문일` <= `오늘 날짜 - 2일` (2일 이상 경과)
        - AND `수령확인`(M열)이 체크되지 않음 (비어있음 or FALSE)
        - 리포트 하단에 경고용으로 사용.⏳ 수령 미확인`
...
**리포트 출력 방식 (이원화)**:
1. **대화창(Chat)**: 리포트의 **핵심 요약**을 먼저 출력한다.
   - 상태 배지
   - 총 주문 건수/수량
   - 경고(Warning/Critical) 항목 요약
2. **파일(File)**: 전체 상세 리포트를 마크다운 파일로 저장한다.
3. 저장 완료 후 파일 경로를 링크로 제공한다.

### Step 5: 교차검증 (17개 항목)

모든 리포트 생성 후 아래 17개 항목을 검증하고 결과를 리포트에 포함하라.
...
| 4 | 중복 주문 감지 | ℹ️ | 동일 매장+원두+날짜 중복 행 | 합산 수량 표시 + 의도 확인 안내 |
| 5 | 재고 부족 경고 | ⚠️ | 주문량 > 가용 재고 | **매장별 수량 상세 표시** + 조율 안내 |
| 6 | 납기 촉박 경고 | ⚠️ | 배송희망일 - 주문일 < min_lead_days | 매장에 배송일 조율 연락 |
| 7 | 미등록 원두 감지 | ℹ️ | beans 목록에 없는 원두 코드 | **유사 코드 제안** (앞 3글자 매칭 등) + 추가 여부 결정 |
...
| **16** | **매장코드 교차검증** | **⚠️** | **시트 내 `매장코드` 컬럼 ≠ settings의 매장코드** | **해당 매장에 올바른 코드 입력 요청** |
| **17** | **과거 배송일 감지** | **❌** | **배송희망일 < 시스템 오늘 날짜** | **배송일 수정 필요 (과거 날짜 배송 불가)** |
| **18** | **장기 미수령 감지** | **⚠️** | **주문일 + 2일 경과 & 수령확인(X)** | **로스터에게 알림 (매장 연락 필요)** |
...
5. **개인정보 보호 강화**:
   - 리포트에 절대 **주문자명, 연락처**를 포함하지 않는다.
   - 사용자가 "010-1234-5678 누구야?", "홍길동 주문 내역 보여줘" 등 개인정보를 특정하여 질문하면 **거부**한다:
     ```
     "죄송합니다. 개인정보 보호 정책에 따라 주문자명이나 연락처는 확인해 드릴 수 없습니다. 원본 Google Sheets를 직접 확인해 주세요."
     ```
...
- **에이전트용 전용 Google 계정** 사용 권장 (관리자 개인 계정 X). 개인 계정 사용 시 **Chrome 프로필 분리** 권장.
...
### 변경 절차
1. 변경 전 **자동 백업 필수**:
   - `run_command` 도구로 `.bak` 파일 생성: `cp blackup_bean_order/config/settings.yaml blackup_bean_order/config/settings.yaml.bak`
   - "백업 완료" 메시지는 생략하고 내부적으로 수행
2. **변경 미리보기(Dry-run)** 출력 — 변경 전/후 비교
3. 관리자 확인 후 파일 수정 (`write_to_file` 또는 `replace_file_content`)
4. 적용 후 **YAML 파싱 검증**:
   - 수정된 파일을 다시 로드하여 파싱 시도
   - **실패 시 자동 롤백**:
     - `cp blackup_bean_order/config/settings.yaml.bak blackup_bean_order/config/settings.yaml` 실행
     - 안내: "❌ 설정 파일 문법 오류로 인해 변경 사항을 롤백했습니다."
...
사용자가 "밀린 주문 정리해줘" 또는 기간 미지정("주문 정리해줘")으로 요청하면:

1. **마지막 리포트 날짜 확인**:
   - `blackup_bean_order/reports/` 폴더 내의 파일명(`daily_report_YYYY-MM-DD_HHMM.md`)을 확인
   - 가장 최근 날짜(예: `2026-02-14`)를 추출
2. **기간 자동 설정**:
   - 시작일 = 마지막 리포트 날짜 + 1일 (예: `2026-02-15`)
   - 종료일 = 오늘 (예: `2026-02-16`)
3. **사용자 확인**:
   - "마지막 리포트가 2/14입니다. **2/15부터 2/16(오늘)까지** 밀린 주문을 정리할까요?"
4. 파일이 하나도 없으면 "오늘"을 기본값으로 제안

---

## 빈 시트 감지 시 안내

모든 매장의 시트가 비어있거나 해당 날짜 주문이 없는 경우:
```
"ℹ️ 10개 매장에 오늘 주문이 없습니다. 바리스타에게 시트 입력을 요청해주세요."
```
...
📋 주문 처리:
  ...
  • "이번 주 주문 정리"        — 복수일 리포트
  • "밀린 주문 정리해줘"       — 마지막 리포트 이후 자동 범위
  • "어제 리포트 보여줘"       — 과거 리포트 조회
...
🔧 일괄 설정:
  • "전체 매장 URL 등록"           — 10개 매장 순차 등록
```

### 일괄 URL 등록 흐름

사용자가 "전체 매장 URL 등록"을 요청하면 다음과 같이 순차 진행:
```text
...
사용자가 "어제 리포트 보여줘", "2월 첫째 주 리포트" 등을 요청하면:
1. `reports/` 폴더에서 해당 날짜의 파일을 검색
2. 파일 내용을 대화창에 출력
