...
**리포트 상단에 처리 상태 배지**를 표시한다:
```
✅ 10/10 매장 처리 성공
— 또는 —
✅ 8/10 매장 처리 | ❌ 2매장 실패 (을숙도, 울산옥동)
```

**동일 날짜 리포트가 이미 존재하면** 사용자에게 확인:
```
"오늘 17:30에 생성된 리포트가 있습니다. 새로 생성할까요?"
```
...
- **원두+로스팅 조합별 합산** + 매장별 소계: `ETH-YRG 미디엄 35kg (SM 15, HD 10, YS 10)`
- 배송 일정: 배송일 + 구역(zone) 기준 정렬 + **매장 주소 포함**
...
| 7 | 미등록 원두 감지 | ℹ️ | beans 목록에 없는 원두 코드 | **유사 코드 제안** (있으면) + 추가 여부 결정 |
...
| **16** | **매장코드 교차검증** | **⚠️** | **시트 소속 매장코드 ≠ 행 내 매장코드** | **해당 매장에 코드 수정 요청** |
| **17** | **과거 배송일 감지** | **❌** | **배송희망일 < 오늘 날짜** | **배송일 수정 필요 — 물리적 배송 불가** |
...
5. **개인정보 보호**: 사용자가 주문자명·연락처를 질문하면 **거부**한다:
   ```
   "개인정보 보호 정책상 리포트에 포함하지 않습니다. 원본 시트를 확인해주세요."
   ```
...
1. 변경 전 **자동 백업**: `.settings.yaml.bak` 파일 생성
2. **변경 미리보기(Dry-run)** 출력 — 변경 전/후 비교
3. 관리자 확인 후 적용
4. 적용 후 **YAML 파싱 검증** — 실패 시 자동 롤백 + 안내:
   ```
   "❌ 설정 파일이 손상되었습니다. 마지막 백업에서 복원할까요?"
   ```
5. 주문 처리 중에는 자동 변경 **금지** — 관리자 명시적 요청 시에만
...
사용자가 "밀린 주문 정리해줘" 또는 기간 미지정으로 요청하면:
1. `reports/` 폴더에서 **마지막 리포트의 날짜**를 확인
2. 마지막 리포트 다음날 ~ 오늘까지를 자동 범위로 설정
3. 사용자에게 범위를 확인: "2/14~2/16 주문을 정리할까요?"
